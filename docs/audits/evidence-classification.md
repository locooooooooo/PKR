...
| --- | --- | --- |
| `automated_fake_provider` | A legacy-labeled checked-in fixture exercised an optional Adapter protocol path | A real external Agent host, production model, user, or product dependency succeeded |
...
Current automated suites use fake process fixtures and disposable repository
fixtures. The `automated_fake_provider` label follows the experimental source
contract name; it is not PKR's product category. No real-Agent audit or
newcomer trial is claimed by those suites.
